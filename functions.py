...
class QConvAutograd(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[1]:
            ...
            # !!! originally here was: grad_weight = torch.nn.grad.conv2d_weight(
            grad_weight_r = torch.nn.grad.conv2d_weight(
                input, weight.shape, grad_output, stride, padding, dilation, groups
            )
            # !!! originally here was: a, _b, _c, _d = torch.chunk(grad_weight_r[:, :weight.size(1) // 4], 4, 0)
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0] and ctx.needs_input_grad[1]:
            with torch.enable_grad():
                grad_input_r = torch.autograd.grad(grad_output_r, weight_r, grad_output_r)
